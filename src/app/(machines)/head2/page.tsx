...
export default function Alpha() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch('/api/submission', {
      ...
      body: JSON.stringify({
        taskId: 'head2',  // Replace with actual task ID
    ...
    <section>
      <div className="flex flex-col justify-center items-center">
        <div className="max-w-7xl mx-auto text-center my-32">
          <h2 className="scroll-m-20 text-5xl font-extrabold tracking-tight lg:text-5xl">Head2</h2>
        ...
        <div className="md:grid md:grid-cols-12 md:gap-6 items-center my-32">
          {/* Image */}
          <ImageSection src="https://ds-security-web.s3.amazonaws.com/ui/media/images/head2.jpg" alt="beerus Image"width={450} height={305} />
          <div className="max-w-xl md:max-w-none md:w-full mx-auto md:col-span-7 lg:col-span-6" data-aos="fade-left">
            <div className="md:pl-4 lg:pl-12 xl:pl-16">
            <Link href={session?.user ? "https://drive.proton.me/urls/WPH7XWXC98#TTfaL1aFH4Qm" :"/signin"} target={session?.user ? "_blank" : ""}>
                  {/* <FileDownloadRoundedIcon /> */}
                  <div className="font-e text-3xl font-bold text-green-600 mb-2">No Sweat</div> 
                  

                  <h3 className="h4 mb-3 w-full">Click to download  <FaDownload className=""/></h3> 
                  
              
                </Link>
              
              <p className="leading-7 text-l mr-56 mb-3 [&:not(:first-child)]:mt-6">Download the OVA file, import it into your virtual machine environment, start the machine, and run</p>
              <code className="relative rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-l font-semibold">`nmap -sn (your-ip-subnet)`</code>
              {session?.user && (
